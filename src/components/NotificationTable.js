...
const NotificationTable = () => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const response = await axios.post(`${base_url}admin/delete/notification`, { id }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    return (
      <div className="p-8 ml-64 bg-white w-full">
        ...
        <Link to='/add/notification'><button className='bg-gray-200 px-4 py-2 rounded-lg shadow-md hover:bg-green-300 flex justify-center items-center'> <AiOutlinePlus /> Add notification</button></Link>
        </div>
                ...
                {products.map((product, index) => (
                  <tr key={index} >
                   <td className="px-3 py-1 border-b border-gray-200 text-sm">
                      <img src={product.image} alt={product.title_en} className="w-36 h-36  "/>
                    </td>
